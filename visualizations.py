"""
visualizations.py — Generate all figures for Report 1.
"""
...
def fig4_correlation_heatmap(df):
    """Correlation heatmap of clinical and affect measures."""
...
def fig5_valence_arousal(df):
    """Valence and arousal box plots by video."""
    rows = []
...
def fig6_presence(df):
    """Presence (immersion) violin plots by video."""
    rows = []
    for vid in VIDEO_ORDER:
        for _, row in df.iterrows():
            rows.append({'Video': VIDEO_NAMES[vid],
...
def fig7_phq_vs_gad(df):
    """PHQ-9 vs GAD-7 scatter with regression line."""
...
def fig8_headtrack_speed_violin(merged):
    """Mean rotation speed violin by video and depression group."""
...
def fig9_headtrack_sd_box(merged):
    """SD of rotation speed box plot by video and depression group."""
    fig, ax = plt.subplots(figsize=(10, 5))
...
def fig10_angular_range(merged):
    """Total angular range explored bar plot by video and depression group."""
    fig, ax = plt.subplots(figsize=(10, 5))
...
def fig11_panas_change(df):
    """PANAS pre/post grouped bar chart."""
...
def fig12_yaw_speed_by_group(merged):
    """Mean yaw speed box plots per video and depression group."""
